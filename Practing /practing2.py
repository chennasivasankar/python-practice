...
stage_moving_from_ready = task_dict_utils.get_field_response(
    ...
    gof_id="CANDIDATES_SELECTION_FINAL_EVALUATION_RATING_GOF",
    gof_order=0,
    field_id="CS_FINAL_VIDEO_SUBMISSION_MOVE_TO_FIELD"
)

video_evaluation_score = total_score_of_first_evelation_score(
    task_dict_utils=task_dict_utils
)
video_evaluation_score_2 = get_2nd_video_evaluation_average_score(
    task_dict_utils=task_dict_utils
)
average_score = get_final_video_evaluation_average_score(
    task_dict_utils=task_dict_utils
)
...
task_dict_utils.update_or_create_field_with_response(
    parent_gof_id=None,
    parent_gof_order=None,
    gof_id="CS_FINAL_EVALUATION_TOTAL_SCORE_GOF",
    gof_order=0,
    field_id="CANDIDATES_SELECTION_FINAL_VID_AVERAGE_RATINGS_FIELD",
    response=average_score
)
total_average = 0
div_average_1and_2 = 0
if video_evaluation_score:
    total_average += video_evaluation_score
    div_average_1and_2 += 1
if video_evaluation_score_2:
    total_average += video_evaluation_score_2
    div_average_1and_2 += 1
if average_score:
    total_average += average_score
    div_average_1and_2 += 1
if total_average > 0:
    overall_rating = round(total_average/div_average_1and_2, 2)
else:
...
task_dict_utils.update_or_create_field_with_response(
    ...
    gof_id="CS_FINAL_EVALUATION_TOTAL_SCORE_GOF",
    gof_order=0,
    field_id="CANDIDATES_SELECTION_FINAL_VID_OVERALL_RATING_FIELD",
...
elif stage_moving_from_ready == "Interview Invitation Call":
